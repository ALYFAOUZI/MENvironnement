...
excel_data_df = pd.read_excel('Sys_Fonc_Travail v02.xlsx', sheet_name='Fonc_Aff',index_col =None,header=None)
...
for k in range(0,len(excel_data_df)):
       for j in excel_data_df.columns:
         ...
         if (excel_data_df[j][k] == 'X' or not excel_data_df.isnull()[j][k] ) and excel_data_df[j][k] != 'nan' and excel_data_df[j][k] != 'na' and not excel_data_df.isnull()[2][k] :
         #'AP - Aires protégées':
            ...
            if not excel_data_df.isnull()[j][0] and excel_data_df.astype(str)[j][0]  !=' ' and excel_data_df.astype(str)[0][k] + '_' + excel_data_df.astype(str)[1][k] != 'nan_nan':
              ...
              {'Domain':  excel_data_df.astype(str)[0][k] + '_' + excel_data_df.astype(str)[1][k] + '_'+ excel_data_df[2][k] ,
                     'System':excel_data_df[j][0] , 'Value':excel_data_df.astype(str)[j][k] }
              )
